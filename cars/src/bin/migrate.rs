...
async fn main() -> anyhow::Result<()> {
    ...
    for migration_file in &migration_files {
        ...
        if already_applied {
            // Double-check: verify that the migration actually created the expected tables
            // For cars service migrations
            let should_reapply = if version == "001_create_tariffs_table" {
                let tariffs_exists: bool = sqlx::query_scalar(
                    "SELECT EXISTS (
                ...
                .await?;
                !tariffs_exists
            } else if version == "002_create_cars_table" {
                let cars_exists: bool = sqlx::query_scalar(
                    "SELECT EXISTS (
                        SELECT FROM information_schema.tables 
                        WHERE table_schema = 'public' 
                        AND table_name = 'cars'
            ...
            if should_reapply {
                println!("Warning: Migration {} marked as applied but expected table doesn't exist. Re-applying...", version);
                // Remove the migration record to allow re-application
